...
async function getMovies() {
  if (DEBUG) console.log("mngr.mongo.dal.getMovies()");
  try {
    await dal.connect();
...
async function getMoviesByMovieId(id) {
  if (DEBUG) console.log("mngr.mongo.dal.getMoviesByMovieId()");
  try {
    await dal.connect();
    const result = dal
      .db("sample_mflix")
...
async function addMovie(genres, title, rated, year) {
  if (DEBUG) console.log("mngr.mongo.dal.addMovie()");
...
async function putMovie(id, genres, title, rated, year) {
  if (DEBUG) console.log("mngr.mongo.dal.putMovie()");
  try {
    await dal.connect();
    const result = await dal
      .db("sample_mflix")
...
async function patchMovie(id, genres, title, rated, year) {
  if (DEBUG) console.log("mngr.mongo.dal.patchMovie()");
  try {
    await dal.connect();
    const result = await dal
      .db("sample_mflix")
...
async function deleteMovie(id) {
  if (DEBUG) console.log("mngr.mongo.dal.deleteMovie()");
  try {
    await dal.connect();
    const result = dal
      .db("sample_mflix")
